...
#Create 10 folds
folds <- cut(seq(1,nrow(movies)),breaks=10,labels=FALSE)

#Perform 10 fold cross validation
for(i in 1:10){
    testIndexes <- which(folds==i,arr.ind=TRUE)
    testData <- movies[testIndexes, ]
    trainData <- movies[-testIndexes, ]
    ...
    matrix <- table(testData$Class, y_pred)
}
```
...
movies.nb <- train(movies.nb.x, movies.nb.y, method = "nb", trControl = movies.ctrl, usekernal=TRUE
)
...
movies.knn.auc
#movies.nb.auc(movies.test$isSuccess, predicted)
```
