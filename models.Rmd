...
for(i in 1:10){
    ...
    classifier <- naiveBayes(isSuccess ~ budget + popularity + runtime + isEN + isCollection + Comedy + `Science Fiction` + Horror + Romance + Action + Thriller + Drama + Mystery + Crime + Animation + Adventure + Fantasy, data = trainData)
    movies.nb.predict <- predict(classifier, newdata = testData)
...
# movies.nb.test <- movies.test[c('isSuccess','budget', 'popularity', 'runtime', 'isEN', 'isCollection', 'Comedy', 'Science Fiction', 'Horror', 'Romance', 'Action', 'Thriller', 'Drama', 'Mystery', 'Crime', 'Animation', 'Adventure', 'Fantasy')]
# movies.nb <- train(movies.nb.x, movies.nb.y, method = "nb", trControl = movies.ctrl)
# movies.nb.predict <- predict(movies.nb, newdata = movies.nb.test)
# movies.nb.predict.bin <- ifelse(movies.nb.predict > 0.5, 1, 0)
# confusionMatrix(as.factor(movies.nb.predict.bin), reference = as.factor(movies.nb.test$isSuccess))
```
## ROC Plot
