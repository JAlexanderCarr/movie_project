...
movies_new <- data.frame(movies_orig$original_title,movies_orig$belongs_to_collection,as.numeric(movies_orig$budget), as.numeric(movies_orig$revenue), as.numeric(movies_orig$popularity), movies_orig$original_language, movies_orig$runtime, movies_orig$release_date, movies_orig$genres)
...
movies_intermediate <- filter(movies_new, revenue > 0 | budget > 0)
movies <- filter(movies_intermediate, as.Date(release_date, format="%m/%d/%Y") >= "01/01/1950")
